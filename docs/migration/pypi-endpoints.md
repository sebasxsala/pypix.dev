...
- Algolia is the beta hosted search provider. `/api/pypi/search` keeps the local Simple/Index search as a fallback and only uses Algolia server-side when the request asks for `provider=algolia` and search credentials are configured. The manual indexer reads PyPI Simple JSON, deduplicates by PEP 503-normalized project name, hydrates PyPI JSON metadata, and writes records to `pypix_packages` without using deprecated PyPI JSON download counters. The default run targets 90,000 saved records with `PYPI_INDEX_MODE=balanced`, seeding known important packages first and then filling from balanced prefix buckets so the free 100,000-record production tier has margin. Some Simple API project names have no files/versions and return 404 from the JSON API, so the indexer treats the target as saved records rather than checked candidates. Use `PYPI_INDEX_MODE=refresh-listed` with `PYPI_INDEX_PROJECTS_FILE` when the job should only refresh the packages already listed in the seed file and avoid adding new packages.
